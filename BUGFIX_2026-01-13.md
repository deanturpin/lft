...
```
15:11:41 - Cycle 1:
  ├─ Place ETH/USD order ($1000)
  ├─ Order fills instantly
  ├─ Position opens in Alpaca
  └─ Local tracking: position_strategies["ETH/USD"] = "mean_reversion"

15:12:39 - Cycle 2 (58 seconds later):
  ├─ API call: get_positions() → DOESN'T include ETH/USD (lag!)
  ├─ Cleanup loop: "ETH/USD not in API, must be stale" → ERASES tracking
  ├─ Trade check: No API position, no local tracking → "Safe to trade"
  └─ Place ANOTHER ETH/USD order ($1000) ❌

15:13:38 - Cycle 3 (59 seconds later):
  └─ Same sequence → THIRD duplicate order ❌
...
for (auto it = position_strategies.begin(); it != position_strategies.end();) {
  if (not api_positions.contains(it->first)) {
    it = position_strategies.erase(it);  // ❌ Too aggressive!
```
...
if (not symbols_in_use.contains(symbol)) {
  place_order();  // ❌ symbols_in_use destroyed each cycle!
}
```
...
if (not symbols_in_use.contains(symbol) and
  ...
  place_order();  // ✓ Protected by persistent map
}
```
...
symbols_in_use.insert(symbol);  // ← NEW: Within-cycle protection
...
15:11:41 - Cycle 1:
  ├─ Place ETH/USD order
  ├─ position_strategies["ETH/USD"] = "mean_reversion"
  ├─ position_entry_times["ETH/USD"] = 15:11:41
  └─ symbols_in_use.insert("ETH/USD")

15:12:39 - Cycle 2:
  ├─ Cleanup check: position_entry_times shows 15:11:41 (58 seconds ago)
  ├─ 58 seconds < 5 minutes → KEEP in position_strategies ✓
  ├─ Trade check: position_strategies.contains("ETH/USD") → TRUE
  └─ BLOCKED - no duplicate order! ✓

15:13:38 - Cycle 3:
  └─ Same protection - still blocked ✓
```
...
1. Monitor console output for "📊 OPEN POSITIONS" table accuracy
...
- `src/alpaca_client.cxx` - Added read timeouts to all API methods
- `include/alpaca_client.h` - No changes

## Status

Fixes committed in f127f4a. Successfully deployed and tested.

## GitHub Issue Tracking

All outstanding issues tracked in GitHub Issues (not local markdown). Key issues related to this bugfix:

- Issue #19: Trade limits and cooldowns (related to duplicate prevention)
- Issue #20: Regime classification (related to strategy performance)
- Issue #23: Time-based exits (related to exit logic improvements)
...
1. **Connection timeout**: How long to wait for TCP handshake (CONFIGURED: 10s ✅)
2. **Read timeout**: How long to wait for response body after connection established (NOT CONFIGURED: INFINITE ❌)

### Failure Scenario

```
1. Program calls Alpaca API (e.g., get_positions())
2. TCP connection succeeds within 10 seconds ✅
3. Alpaca's server accepts connection but is slow/stalled responding
   (high load, rate limiting, internal issues, etc.)
4. Program waits FOREVER for response body ❌
