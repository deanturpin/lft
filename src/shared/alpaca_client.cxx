...
AlpacaClient::get_snapshots(const std::vector<std::string>& symbols) {
    ...
    if (res->status != 200) {
        ...
        std::println(stderr, "Snapshots API error: status={}, body={}", res->status, res->body);
...
AlpacaClient::get_crypto_snapshots(const std::vector<std::string>& symbols) {
    ...
    auto path = std::format("/v1beta3/crypto/us/snapshots?symbols={}", symbol_list);
    ...
    if (res->status != 200) {
        // Debug: print error response
        std::println(stderr, "Crypto snapshots API error: status={}, body={}", res->status, res->body);
        return std::unexpected(AlpacaError::UnknownError);
    }
...
std::expected<std::string, AlpacaError> AlpacaClient::place_order(
    ...
    auto res = client.Post("/v2/orders", headers, order.dump(), "application/json");
    ...
    if (res->status == 403 or res->status == 422) {
        std::println(stderr, "Order rejected: status={}, body={}", res->status, res->body);
        return std::unexpected(AlpacaError::UnknownError);
    }

    if (res->status != 200) {
        std::println(stderr, "Order API error: status={}, body={}", res->status, res->body);
        return std::unexpected(AlpacaError::UnknownError);
    }
...
std::expected<std::string, AlpacaError> AlpacaClient::close_position(std::string_view symbol) {
    auto client = httplib::Client{base_url_};
    client.set_connection_timeout(10);
    ...
    if (res->status != 200) {
        std::println(stderr, "Close position error: status={}, body={}", res->status, res->body);
        return std::unexpected(AlpacaError::UnknownError);
    }
