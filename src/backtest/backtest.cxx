...
constexpr auto take_profit_threshold = 0.0;  // Any profit
constexpr auto stop_loss_amount = -10.0;     // -$10 loss
...
void print_summary(const BacktestStats& stats) {
    ...
    std::println("Final Capital:   {}{:.2f}{} ({:+.2f}%)",
                 pl_colour, stats.cash, colour_reset, return_pct);
    std::println("Total P&L:       {}{:+.2f}{}",
                 pl_colour, total_pl, colour_reset);
    std::println("Total Trades:    {}", stats.total_trades);
    std::println("Win Rate:        {:.1f}% ({}/{} wins)",
                 stats.total_trades > 0 ? (stats.winning_trades * 100.0 / stats.total_trades) : 0.0,
...
void process_bar(const std::string& symbol,
                const lft::Bar& bar,
    ...
    if (has_position) {
        ...
        if (unrealized_pl > take_profit_threshold or unrealized_pl < stop_loss_amount) {
            // Close position
