...
std::map<std::string, std::string> position_strategies{};  // symbol -> strategy name
std::map<std::string, double> position_peaks{};            // symbol -> peak price for trailing stop
...
MarketAssessment assess_market_conditions(AlpacaClient &client, const std::vector<Snapshot> &snapshots) {
  if (snapshots.empty())
  ...
  auto summary = std::format("{} {} of {} symbols tradeable (avg spread: {:.1f} bps)\n",
  ...
  if (symbols.size() > display_count)
    summary += std::format("  ... {} more symbols\n", symbols.size() - display_count);
...
std::map<std::string, std::vector<Bar>> fetch_bars(AlpacaClient &client) {
  ...
  std::println("  Fetching {} days of 15-min bars for {} symbols...", calibration_days, stocks.size());
  ...
  for (const auto &symbol : stocks) {
    if (auto bars = client.get_bars(symbol, "15Min", calibration_days)) {
      ...
      ++fetched;
      std::println("    {}/{}: {} ({} bars)", fetched, stocks.size(), symbol, bars->size());
    }
  }
